...
$("#carousel-banner").bind("resize", function() {
  if (window.innerWidth < 992) {
    var height = $("#carousel-banner").height();
    var height1 = $("#carousel-mid-banner").height();
    if (height && height1) {
      var scrollTop = height + height1 + 121;
      console.log(height, height1, scrollTop);
...
function noticeList(ele, data) {
  ...
  data.forEach(function(item, index) {
    html +=
      '<li>\
                    <a class="notice-li ellipsis" href="/notice-details.html?id=' +
      item.id +
      "&type=0&number=" +
      item.number +
      '" target="_self">' +
...
function renderList(ele, data, type) {
  ...
  data.forEach(function(item, index) {
    html +=
      '<li>\
                    <a class="clearfix ellipsis" href="/new-details.html?id=' +
      item.id +
      "&type=" +
      item.type +
      '" target="_self">' +
